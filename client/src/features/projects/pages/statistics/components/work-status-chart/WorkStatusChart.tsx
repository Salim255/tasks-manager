...
export const WorkStatusChart = ({
  ...
  circleChartDataPercentage: Record<
    TaskStatus,
    { label: TaskStatus; nb: number; value: number }
  >;
}) => {
  ...
  const isEmpty = statusData.every((d) => d.value === 0);
  ...
  return (
    <div className="work-status-chart">
      ...
      {isEmpty ? (
        <div className="work-status-chart__empty">
          No status data yet
        </div>
      ) : (
